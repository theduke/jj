...
fn test_git_push_undo() {
    ...
    // bookmarkes, but that also has undersired consequences: the second fetch in
...
fn test_git_push_undo_with_import() {
    ...
    // remote-tracking bookmarkes should happen on every operation.
...
// both the git_refs and the remote-tracking bookmarkes are preserved by undo.
...
fn test_git_push_undo_repo_only() {
    ...
    // Undo the push, but keep both the git_refs and the remote-tracking bookmarkes
    test_env.jj_cmd_ok(
        &repo_path,
