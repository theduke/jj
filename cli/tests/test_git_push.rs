...
fn test_git_push_current_bookmark() {
    ...
    // Update some bookmarkes. `bookmark1` is not a current bookmark, but
    ...
    // Check the setup
    insta::assert_snapshot!(get_bookmark_output(&test_env, &workspace_root), @r###"
    bookmark1: xtvrqkyv 96cc58bf (empty) modified bookmark1 commit
    ...
    insta::assert_snapshot!(stdout, @"");
    insta::assert_snapshot!(stderr, @r###"
    Warning: No bookmarkes found in the default push revset: remote_bookmarkes(remote=origin)..@
    Nothing changed.
    "###);
...
fn test_git_push_parent_bookmark() {
    ...
    insta::assert_snapshot!(stderr, @r###"
    Branch changes to push to origin:
      Move sideways bookmark bookmark1 from d13ecdbda2a2 to 78bdfdea51c7
    "###);
}
...
/// (`remote_bookmarkes(remote=<remote>)..@` vs. `remote_bookmarkes()..@`).
#[test]
...
// This tests whether the push checks that the remote bookmarkes are in expected
// positions.
#[test]
...
fn test_git_push_creation_unexpectedly_already_exists() {
    ...
    insta::assert_snapshot!(get_bookmark_output(&test_env, &workspace_root), @r###"
    bookmark1: yostqsxw 4c595cf9 new bookmark1
    bookmark2: rlzusymt 8476341e (empty) description 2
      @origin: rlzusymt 8476341e (empty) description 2
    "###);
    ...
    insta::assert_snapshot!(stderr, @r###"
    Branch changes to push to origin:
      Add bookmark bookmark1 to 4c595cf9ac0a
    Error: Refusing to push a bookmark that unexpectedly moved on the remote. Affected refs: refs/heads/bookmark1
    Hint: Try fetching from the remote, then make the bookmark point to where you want it to be, and push again.
    "###);
}
...
fn test_git_push_locally_created_and_rewritten() {
    let (test_env, workspace_root) = set_up();
    // Ensure that remote bookmarkes aren't tracked automatically
...
fn test_git_push_multiple() {
    ...
    Dry-run requested, not pushing.
    "###);
    // Dry run requesting two specific bookmarkes
    ...
    Dry-run requested, not pushing.
    "###);
    // Dry run requesting two specific bookmarkes twice
    let (stdout, stderr) = test_env.jj_cmd_ok(
        &workspace_root,
        &[
            "git",
    ...
    insta::assert_snapshot!(stderr, @r###"
    Error: No matching bookmarkes for patterns: foo, ?bookmark
    "###);
...
fn test_git_push_revisions() {
    ...
    insta::assert_snapshot!(stderr, @r###"
    Warning: No bookmarkes point to the specified revisions: none()
    Nothing changed.
    "###);
    // Push a revision with no bookmarkes
    let (stdout, stderr) = test_env.jj_cmd_ok(&workspace_root, &["git", "push", "-r=@--"]);
    insta::assert_snapshot!(stdout, @"");
    insta::assert_snapshot!(stderr, @r###"
    Warning: No bookmarkes point to the specified revisions: @--
    Nothing changed.
    "###);
    ...
    Dry-run requested, not pushing.
    "###);
    // Push multiple revisions of which some have bookmarkes
    let (stdout, stderr) = test_env.jj_cmd_ok(
        &workspace_root,
    ...
    Dry-run requested, not pushing.
    "###);
    // Push a revision with a multiple bookmarkes
    let (stdout, stderr) =
        test_env.jj_cmd_ok(&workspace_root, &["git", "push", "-r=@", "--dry-run"]);
    insta::assert_snapshot!(stdout, @"");
    insta::assert_snapshot!(stderr, @r###"
...
#[test]
fn test_git_push_conflicting_bookmarkes() {
    let (test_env, workspace_root) = set_up();
...
fn test_git_push_tracked_vs_all() {
    ...
    insta::assert_snapshot!(get_bookmark_output(&test_env, &workspace_root), @r###"
    bookmark1: vruxwmqv 0b16bb02 (empty) moved bookmark1
    bookmark1@origin: xtvrqkyv d13ecdbd (empty) description 1
    bookmark2 (deleted)
      @origin: rlzusymt 8476341e (empty) description 2
    bookmark3: znkkpsqq 998d6a78 (empty) moved bookmark2
    "###);

    // At this point, only bookmark2 is still tracked. `jj git push --tracked` would
    // try to push it and no other bookmarkes.
    let (_stdout, stderr) =
    ...
    // Now, no bookmarkes are tracked. --tracked does not push anything
    let (_stdout, stderr) = test_env.jj_cmd_ok(&workspace_root, &["git", "push", "--tracked"]);
    insta::assert_snapshot!(stderr, @r###"
    Nothing changed.
    "###);

    // All bookmarkes are still untracked.
    ...
    //   currently does *not* list bookmarkes like bookmark2 as "about to be
    ...
    Branch changes to push to origin:
      Add bookmark bookmark3 to 998d6a7853d9
    "###);
}
