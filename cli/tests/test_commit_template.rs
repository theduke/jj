...
fn test_log_builtin_templates_colored() {
    ...
    │  Branches: [38;5;5mmy-bookmark[39m
...
fn test_log_builtin_templates_colored_debug() {
    ...
    insta::assert_snapshot!(render(r#"builtin_log_compact"#), @r###"
    [1m[38;5;2m<<node working_copy::@>>[0m  [1m[38;5;13m<<log working_copy change_id shortest prefix::r>>[38;5;8m<<log working_copy change_id shortest rest::lvkpnrz>>[39m<<log working_copy:: >>[38;5;9m<<log working_copy email placeholder::(no email set)>>[39m<<log working_copy:: >>[38;5;14m<<log working_copy committer timestamp local format::2001-02-03 08:05:08>>[39m<<log working_copy:: >>[38;5;13m<<log working_copy bookmarkes name::my-bookmark>>[39m<<log working_copy:: >>[38;5;12m<<log working_copy commit_id shortest prefix::d>>[38;5;8m<<log working_copy commit_id shortest rest::c315397>>[39m<<log working_copy::>>[0m
    ...
    │  <<log::Branches: >>[38;5;5m<<log local_bookmarkes name::my-bookmark>>[39m<<log::>>
...
fn test_log_bookmarkes() {
    ...
    // Created some bookmarkes on the remote
    ...
    let template = r#"commit_id.short() ++ " " ++ if(bookmarkes, bookmarkes, "(no bookmarkes)")"#;
    ...
    ├─╯
    │ ○  28ff13ce7195 bookmark1*
    ├─╯
    │ @  a5b4d15489cc bookmark2* new-bookmark
    │ ○  8476341eb395 bookmark2@origin unchanged
    ├─╯
    ◆  000000000000 (no bookmarkes)
    "###);

    let template = r#"bookmarkes.map(|b| separate("/", b.remote(), b.name())).join(", ")"#;
    let output = test_env.jj_cmd_success(&workspace_root, &["log", "-T", template]);
    insta::assert_snapshot!(output, @r###"
    ...
    let template = r#"separate(" ", "L:", local_bookmarkes, "R:", remote_bookmarkes)"#;
    let output = test_env.jj_cmd_success(&workspace_root, &["log", "-T", template]);
    insta::assert_snapshot!(output, @r###"
    ...
    let template = r#"
    remote_bookmarkes.map(|ref| concat(
      ref,
      if(ref.tracked(),
    ...
    let output = test_env.jj_cmd_success(
        &workspace_root,
        &["log", "-r::remote_bookmarkes()", "-T", template],
    );
    insta::assert_snapshot!(output, @r###"
