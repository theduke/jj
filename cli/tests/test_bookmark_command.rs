...
fn test_bookmark_multiple_names() {
    ...
    Created 2 bookmarkes pointing to qpvuntsm 230dd059 bar foo | (empty) (no description set)
    ...
    insta::assert_snapshot!(stdout, @"");
    insta::assert_snapshot!(stderr, @r###"
    Moved 2 bookmarkes to zsuskuln 8bb159bc bar foo | (empty) (no description set)
    Hint: Use -r to specify the target revision.
    "###);
    insta::assert_snapshot!(get_log_output(&test_env, &repo_path), @r###"
    ...
    insta::assert_snapshot!(stderr, @r###"
    Created 1 bookmarkes pointing to zsuskuln 8bb159bc bar baz | (empty) (no description set)
    Moved 1 bookmarkes to zsuskuln 8bb159bc bar baz | (empty) (no description set)
    Hint: Consider using `jj bookmark move` if your intention was to move existing bookmarkes.
    "###);
    ...
    insta::assert_snapshot!(stderr, @r###"
    Moved 1 bookmarkes to zsuskuln 8bb159bc bar baz foo | (empty) (no description set)
    "###);
}
...
fn test_bookmark_at_root() {
    ...
    insta::assert_snapshot!(stdout, @"");
    insta::assert_snapshot!(stderr, @r###"
    Created 1 bookmarkes pointing to zzzzzzzz 00000000 fred | (empty) (no description set)
    "###);
...
fn test_bookmark_move() {
    ...
    insta::assert_snapshot!(stderr, @r###"
    Created 1 bookmarkes pointing to qpvuntsm 230dd059 foo | (empty) (no description set)
    Hint: Consider using `jj bookmark move` if your intention was to move existing bookmarkes.
    "###);
    ...
    let (_stdout, stderr) = test_env.jj_cmd_ok(&repo_path, &["bookmark", "set", "foo"]);
    insta::assert_snapshot!(stderr, @r###"
    Moved 1 bookmarkes to mzvwutvl 167f90e7 foo | (empty) (no description set)
    "###);
    ...
    let stderr = test_env.jj_cmd_failure(&repo_path, &["bookmark", "create", "foo"]);
    insta::assert_snapshot!(stderr, @r###"
    Error: Tracked remote bookmarkes exist for deleted bookmark: foo
    ...
    let (_stdout, stderr) = test_env.jj_cmd_ok(&repo_path, &["bookmark", "set", "foo"]);
    insta::assert_snapshot!(stderr, @r###"
    Moved 1 bookmarkes to mzvwutvl 66d48752 foo* | (empty) (no description set)
    "###);
    insta::assert_snapshot!(get_bookmark_output(&test_env, &repo_path), @r###"
    ...
    insta::assert_snapshot!(stderr, @r###"
    Created 1 bookmarkes pointing to mzvwutvl 66d48752 foo | (empty) (no description set)
    "###);
    insta::assert_snapshot!(get_bookmark_output(&test_env, &repo_path), @r###"
    foo: mzvwutvl 66d48752 (empty) (no description set)
...
fn test_bookmark_move_matching() {
    ...
    // No bookmarkes pointing to the source revisions
    let (_stdout, stderr) = test_env.jj_cmd_ok(&repo_path, &["bookmark", "move", "--from=none()"]);
    insta::assert_snapshot!(stderr, @r###"
    No bookmarkes to update.
    "###);

    // No matching bookmarkes within the source revisions
    let stderr =
        test_env.jj_cmd_failure(&repo_path, &["bookmark", "move", "--from=::@", "glob:a?"]);
    insta::assert_snapshot!(stderr, @r###"
    Error: No matching bookmarkes for patterns: a?
    "###);
    ...
    insta::assert_snapshot!(stderr, @r###"
    Moved 2 bookmarkes to vruxwmqv a2781dd9 b1 c1 | (empty) head2
    Hint: Specify bookmark by name to update just one of the bookmarkes.
    "###);
    insta::assert_snapshot!(get_log_output(&test_env, &repo_path), @r###"
    @  b1 c1 a2781dd9ee37
    ○   f4f38657a3dd
    ...
    // Try to move multiple bookmarkes, but one of them isn't fast-forward
    ...
    insta::assert_snapshot!(stderr, @r###"
    Moved 1 bookmarkes to kkmpptxz 6b5e840e a1 | (empty) head1
    "###);
    insta::assert_snapshot!(get_log_output(&test_env, &repo_path), @r###"
    @   a2781dd9ee37
    ○  c1 f4f38657a3dd
...
fn test_bookmark_move_conflicting() {
    ...
    let repo_path = test_env.env_root().join("repo");
    let get_log = || {
        let template = r#"separate(" ", description.first_line(), bookmarkes)"#;
    ...
    insta::assert_snapshot!(stderr, @r###"
    Moved 1 bookmarkes to mzvwutvl 9328d344 foo | (empty) A1
    "###);
    insta::assert_snapshot!(get_log(), @r###"
    @  A1 foo
    ○  A0
...
fn test_bookmark_rename() {
    ...
    insta::assert_snapshot!(stderr, @r###"
    Warning: Tracked remote bookmarkes for bookmark bremote were not renamed.
    ...
    insta::assert_snapshot!(stderr, @r###"
    Warning: Tracked remote bookmarkes for bookmark bremote exist.
...
fn test_bookmark_rename_colocated() {
    ...
    // Make sure that git tracking bookmarkes don't cause a warning
    let (_stdout, stderr) =
        test_env.jj_cmd_ok(&repo_path, &["bookmark", "rename", "blocal", "blocal1"]);
    insta::assert_snapshot!(stderr, @"");
}
...
fn test_bookmark_forget_glob() {
    ...
    insta::assert_snapshot!(stderr, @r###"
    Error: No matching bookmarkes for patterns: baz*, boom*
    "###);
}
...
fn test_bookmark_delete_glob() {
    ...
    test_env.jj_cmd_ok(&repo_path, &["bookmark", "create", "foo-3"]);
    test_env.jj_cmd_ok(&repo_path, &["bookmark", "create", "foo-4"]);
    // Push to create remote-tracking bookmarkes
    ...
    // We get an error if none of the globs match live bookmarkes. Unlike `jj
    // bookmark forget`, it's not allowed to delete already deleted bookmarkes.
    let stderr = test_env.jj_cmd_failure(&repo_path, &["bookmark", "delete", "glob:foo-[1-3]"]);
    insta::assert_snapshot!(stderr, @r###"
    Error: No matching bookmarkes for patterns: foo-[1-3]
    "###);
    ...
    // The deleted bookmarkes are still there
    insta::assert_snapshot!(get_bookmark_output(&test_env, &repo_path), @r###"
...
fn test_bookmark_forget_export() {
    ...
    Forgot 1 bookmarkes.
    "###);
    // Forgetting a bookmark deletes local and remote-tracking bookmarkes including
...
fn test_bookmark_track_untrack() {
    ...
    let create_remote_commit = |message: &str, data: &[u8], ref_names: &[&str]| {
        ...
        let tree_oid = tree_builder.write().unwrap();
        let tree = git_repo.find_tree(tree_oid).unwrap();
        // Create commit and bookmarkes in the remote
    ...
    // Fetch new commit without auto tracking. No local bookmarkes should be
    // created.
    ...
    // Untrack existing and locally-deleted bookmarkes. Branch targets should be
    // unchanged
...
fn test_bookmark_track_untrack_patterns() {
    ...
    Error: No matching remote bookmarkes for patterns: maine@*
    "###);
    insta::assert_snapshot!(
    ...
    Error: No matching remote bookmarkes for patterns: main@origin, main@o*
    "###);
    ...
    Warning: Remote bookmark not tracked yet: feature2@origin
    Warning: Git-tracking bookmark cannot be untracked: main@git
    Stopped tracking 1 remote bookmarkes.
    "###);
    insta::assert_snapshot!(get_bookmark_output(&test_env, &repo_path), @r###"
    feature1: omvolwpu 1336caed commit
      @git: omvolwpu 1336caed commit
    ...
    insta::assert_snapshot!(stderr, @r###"
    Started tracking 2 remote bookmarkes.
    "###);
    insta::assert_snapshot!(get_bookmark_output(&test_env, &repo_path), @r###"
    feature1: omvolwpu 1336caed commit
      @git: omvolwpu 1336caed commit
...
fn test_bookmark_list() {
    ...
    "###);
    insta::assert_snapshot!(stderr, @r###"
    Hint: Branches marked as deleted will be *deleted permanently* on the remote on the next `jj git push`. Use `jj bookmark forget` to prevent this.
    "###);
...
fn test_bookmark_list_filtered() {
    ...
    let template = r#"separate(" ", commit_id.short(), bookmarkes, if(hidden, "(hidden)"))"#;
    insta::assert_snapshot!(
        test_env.jj_cmd_success(
            &local_path,
            &["log", "-r::(bookmarkes() | remote_bookmarkes())", "-T", template],
        ),
        @r###"
    ...
    // All bookmarkes are listed by default.
    let (stdout, stderr) = test_env.jj_cmd_ok(&local_path, &["bookmark", "list"]);
    insta::assert_snapshot!(stdout, @r###"
    ...
    // "all()" doesn't include deleted bookmarkes since they have no local targets.
    // So "all()" is identical to "bookmarkes()".
    ...
    // Exclude remote-only bookmarkes. "remote-rewrite@origin" is included since
    // local "remote-rewrite" target matches.
    let (stdout, stderr) = query(&["-rbookmarkes()"]);
    insta::assert_snapshot!(stdout, @r###"
    local-keep: kpqxywon c7b4c09c (empty) local-keep
    remote-keep: nlwprzpn 911e9120 (empty) remote-keep
    remote-rewrite: xyxluytn e31634b6 (empty) rewritten
    ...
    "###);
    insta::assert_snapshot!(stderr, @"");
    let (stdout, stderr) = query(&["-rbookmarkes(remote-rewrite)"]);
    insta::assert_snapshot!(stdout, @r###"
    remote-rewrite: xyxluytn e31634b6 (empty) rewritten
      @origin (ahead by 1 commits, behind by 1 commits): xyxluytn hidden 3e9a5af6 (empty) remote-rewrite
    "###);
    ...
    // Select bookmarkes by name, combined with --all-remotes
    ...
    "###);
    insta::assert_snapshot!(stderr, @"");
    let (stdout, stderr) = query(&["--all-remotes", "-rbookmarkes(remote-rewrite)"]);
    insta::assert_snapshot!(stdout, @r###"
    remote-rewrite: xyxluytn e31634b6 (empty) rewritten
      @git: xyxluytn e31634b6 (empty) rewritten
      @origin (ahead by 1 commits, behind by 1 commits): xyxluytn hidden 3e9a5af6 (empty) remote-rewrite
    ...
    Hint: Branches marked as deleted will be *deleted permanently* on the remote on the next `jj git push`. Use `jj bookmark forget` to prevent this.
    "###);
    let (stdout, stderr) = query(&["-rbookmarkes(remote-delete)"]);
    insta::assert_snapshot!(stdout, @r###"
    "###);
    ...
    let (stdout, stderr) = query(&["local-keep", "-rbookmarkes(remote-rewrite)"]);
    insta::assert_snapshot!(stdout, @r###"
    local-keep: kpqxywon c7b4c09c (empty) local-keep
    remote-rewrite: xyxluytn e31634b6 (empty) rewritten
      @origin (ahead by 1 commits, behind by 1 commits): xyxluytn hidden 3e9a5af6 (empty) remote-rewrite
...
fn get_log_output(test_env: &TestEnvironment, cwd: &Path) -> String {
    let template = r#"bookmarkes ++ " " ++ commit_id.short()"#;
