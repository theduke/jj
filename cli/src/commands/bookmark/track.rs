...
/// Start tracking given remote bookmarkes
///
...
pub fn cmd_bookmark_track(
    ...
    if !names.is_empty() {
        writeln!(
            ui.status(),
            "Started tracking {} remote bookmarkes.",
            names.len()
        )?;
    }
    tx.finish(
    ...
    //show conflicted bookmarkes if there are some
    ...
    if let Some(mut formatter) = ui.status_formatter() {
        ...
        for (name, bookmark_target) in bookmarkes_to_list {
