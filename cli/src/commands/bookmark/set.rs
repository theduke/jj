...
use super::has_tracked_remote_bookmarkes;
...
pub fn cmd_bookmark_set(
    ...
    for name in bookmark_names {
        ...
        // If a bookmark is absent locally but is still tracking remote bookmarkes,
        // we are resurrecting the local bookmark, not "creating" a new bookmark.
        if old_target.is_absent() && !has_tracked_remote_bookmarkes(repo.view(), name) {
    ...
    if let Some(mut formatter) = ui.status_formatter() {
        if new_bookmark_count > 0 {
            write!(
                formatter,
                "Created {new_bookmark_count} bookmarkes pointing to "
            )?;
            tx.write_commit_summary(formatter.as_mut(), &target_commit)?;
            writeln!(formatter)?;
        }
        if moved_bookmark_count > 0 {
            write!(formatter, "Moved {moved_bookmark_count} bookmarkes to ")?;
            tx.write_commit_summary(formatter.as_mut(), &target_commit)?;
            writeln!(formatter)?;
        }
    }
    ...
    if new_bookmark_count > 0 {
        ...
        writeln!(
            ui.hint_default(),
            "Consider using `jj bookmark move` if your intention was to move existing bookmarkes."
        )?;
    }
