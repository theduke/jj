...
pub fn cmd_bookmark_create(
    ...
    for name in bookmark_names {
        ...
        if has_tracked_remote_bookmarkes(view, name) {
            return Err(user_error_with_hint(
                format!("Tracked remote bookmarkes exist for deleted bookmark: {name}"),
                format!(
    ...
    if let Some(mut formatter) = ui.status_formatter() {
        write!(
            formatter,
            "Created {} bookmarkes pointing to ",
            bookmark_names.len()
        )?;
