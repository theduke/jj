...
use jj_lib::backend::{BackendError, CommitId, MergedTreeId};
...
fn create_output_files(id: &CommitId, path: &PathBuf) -> Result<(File, File), RunError> {
...
fn create_working_copies(
    repo_path: &Path,
    commits: &[Commit],
) -> Result<Vec<OnDiskCommit>, RunError> {
    let mut results = vec![];
...
/// Try to discover existing `OnDiskCommit`s from a previous invocation.
fn discover_existing_working_copies(_path: &Path) -> Option<Vec<OnDiskCommit>> {
    None
}
...
async fn rewrite_commit<'a>(
    ...
    stored_commit: &mut OnDiskCommit,
    shell_command: &str,
    ...
    // ...
    // ```
    let mut command = tokio::process::Command::new(shell_command)
        // set cwd correctly
        // .arg()
        // TODO: relativize
    ...
    let tree_state = &mut stored_commit.tree_state;
...
pub fn cmd_run(ui: &mut Ui, command: &CommandHelper, args: &RunArgs) -> Result<(), CommandError> {
    ...
    if let Some(existing_commits) = discover_existing_working_copies(&repo_path) {
        // TODO: existing - new commits
    } else {
        // TODO: consider on-demand creation for the inner loop.
        stored_commits = create_working_copies(&repo_path, &resolved_commits)?;
    }
    ...
    tx.finish(ui, "run: rewrote {count} commits");
